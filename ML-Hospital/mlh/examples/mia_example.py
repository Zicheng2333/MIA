...
if __name__ == "__main__":
    ...
    if args.mode == 'prune':
        ...
        checkpoint1 = torch.load(
            f'{args.log_path}/{args.dataset}/{args.training_type}/target/{args.dataset}/{args.mode}/{args.dataset}-global-{args.method}-{args.model}/{args.mode}/{args.model}_{args.dataset}_{args.method}.pth')
        target_model = checkpoint1.to(args.device)
        target_model = torch.nn.DataParallel(target_model)

        # 加载影子模型
        checkpoint2 = torch.load(
            f'{args.log_path}/{args.dataset}/{args.training_type}/shadow/{args.dataset}/{args.mode}/{args.dataset}-global-{args.method}-{args.model}/{args.mode}/{args.model}_{args.dataset}_{args.method}.pth')
